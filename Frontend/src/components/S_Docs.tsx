          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(350px, 1fr))', gap: 24 }}>
            <div style={{ background: '#fff', padding: 20, borderRadius: 12, border: '1px solid #e2e8f0', boxShadow: '0 2px 4px rgba(0,0,0,0.02)' }}>
              ...
              <AddressFields
                ...
                prefix="contact"
                legacyValue={formData.contactAddress}
                onChange={(field, value) => setFormData(prev => {
                  const next = { ...prev, [field]: value };
                  // ประกอบข้อความรวมทันที เอกสาร PDF สร้างจากค่านี้
                  return { ...next, contactAddress: composeAddress(readAddressParts(next as unknown as Record<string, unknown>, "contact")) };
                })}
              />
            ...
            <div style={{ background: '#fff', padding: 20, borderRadius: 12, border: '1px solid #e2e8f0', boxShadow: '0 2px 4px rgba(0,0,0,0.02)' }}>
              ...
              <AddressFields
                ...
                onChange={(field, value) => setFormData(prev => {
                  const next = { ...prev, [field]: value };
                  return { ...next, emergencyAddress: composeAddress(readAddressParts(next as unknown as Record<string, unknown>, "emer")) };
                })}
              />
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
